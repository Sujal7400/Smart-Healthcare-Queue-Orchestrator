...
function getPriorityColor(score) {
  if (score >= 100) return "#e74c3c"; // emergency - red
  if (score >= 80) return "#e67e22";  // high - orange
  if (score >= 50) return "#f1c40f";  // walk-in/medium - yellow
  return "#27ae60"; // appointment/low - green
}
...
function StaffDashboard() {
  ...
  useEffect(() => {
    if (department) fetchQueue(department);

    socket.on("queueUpdated", (data) => {
      if (data.department === department) fetchQueue(department);
    });
  ...
  const handleCreateReferral = async (e) => {
    ...
    try {
      ...
      if (referralForm.referredToDepartment === department) fetchQueue(department);
    } catch (err) {
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.card}>
        ...
        />
        <button style={styles.button} onClick={() => fetchQueue(department)}>Load Queue</button>
      </div>
...
const styles = {
  page: { padding: "30px", background: "#f0f4f8", minHeight: "100vh", fontFamily: "Arial, sans-serif" },
  header: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" },
  logoutBtn: { padding: "8px 16px", background: "#c0392b", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer" },
  message: { background: "#fff3cd", padding: "10px", borderRadius: "6px", marginBottom: "15px" },
  card: { background: "#fff", padding: "20px", borderRadius: "10px", boxShadow: "0 2px 8px rgba(0,0,0,0.1)", marginBottom: "20px" },
  ...
  queueRow: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #eee" },
